...
public class ConsultantVoteResource {
    ...
    /**
     * SEARCH  /_search/consultant-votes?query=:query : search for the consultantVote corresponding
     * to the query.
     *
     * @param query the query of the consultantVote search
     * @return the result of the search
     */
    @GetMapping("/_search/consultant-votes")
    @Timed
    public List<ConsultantVoteDTO> searchConsultantVotes(@RequestParam String query) {
        log.debug("REST request to search ConsultantVotes for query {}", query);
        return consultantVoteService.search(query);
    }
