...
public class DayTimeResource {
    ...
    /**
     * SEARCH  /_search/day-times?query=:query : search for the dayTime corresponding
     * to the query.
     *
     * @param query the query of the dayTime search
     * @return the result of the search
     */
    @GetMapping("/_search/day-times")
    @Timed
    public List<DayTimeDTO> searchDayTimes(@RequestParam String query) {
        log.debug("REST request to search DayTimes for query {}", query);
        return dayTimeService.search(query);
    }
