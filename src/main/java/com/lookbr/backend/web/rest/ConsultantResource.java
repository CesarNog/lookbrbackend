...
public class ConsultantResource {
    ...
    /**
     * SEARCH  /_search/consultants?query=:query : search for the consultant corresponding
     * to the query.
     *
     * @param query the query of the consultant search
     * @return the result of the search
     */
    @GetMapping("/_search/consultants")
    @Timed
    public List<ConsultantDTO> searchConsultants(@RequestParam String query) {
        log.debug("REST request to search Consultants for query {}", query);
        return consultantService.search(query);
    }
